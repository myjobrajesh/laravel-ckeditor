...
trait CkEditorHelpers
{
    //new function to get disk if s3 storage unable.
    public function getDiskToStore() {
        if(config('filesystems.ckeditor_s3_storage')) {
            return Storage::disk('ckeditor_s3');
        }
        return Storage::disk('ckeditor_public');
        //return null;
    }
    ...
    public function getLocalUploadPath($file = null) {
        $folder_name = config('filesystems.disks.ckeditor_public.root');
        $disk = Storage::disk('ckeditor_public');
        //if dir  not exists then create

        if(!$disk->exists($this->ds)) {
            $disk->makeDirectory($this->ds);
        }

        if($file) {
            $folder_name .=$this->ds.$file;
        }
    ...
    {
        $path = $this->composeSegments('url', $is_thumb, $image_name);
        if(config('filesystems.ckeditor_s3_storage')) {
            $path = str_replace(config('lfm.prefix'), '', $path);
            return 'https://s3.amazonaws.com/'.config('filesystems.disks.ckeditor_s3.bucket').$path;
        } else {//for local
            $path = str_replace(config('lfm.prefix'), 'ckeditor', $path);
            return url($path);
        }
    }
    ...
    public function getDirectories($path)
    {
        $disk = $this->getDiskToStore();
            ...
            }, array_filter($disk->allDirectories($path), function ($directory) {
    ...
    */
    ...
    {
        if($disk = $this->getDiskToStore()) {

            return array_map(function ($file) use($disk) {
                return $this->objectPresenter($file, $disk);
            }, $disk->files($path));
        }
     ...
     * @return object
     */
    public function objectPresenter($item, $disk=null)
    {
        if(!$disk) {
            $disk = $this->getDiskToStore();
        }
        $item_name = $this->getName($item);
        $is_file = ($disk) ? $disk->exists($item)  : is_file($item);
        $thumb_url = null;
        if (! $is_file) {
            ...
            if ($disk->exists($thumb_path)) {
                $thumb_url = $this->getThumbUrl($item_name) . '?timestamp=' . $disk->lastModified($thumb_path);
            } else {
                $thumb_url = $this->getFileUrl($item_name) . '?timestamp=' . $disk->lastModified($file_path);
            }

        }
        //echo $disk->lastModified("\\".$item);
        $mtime = $disk->lastModified($item);
        return (object) [
            'name'    => $item_name,
            'url'     => $is_file ? $this->getFileUrl($item_name) : '',
            'size'    => $is_file ? $this->humanFilesize($disk->size($item)) : '',
            'updated' => $mtime,
            'path'    => $is_file ? '' : $this->getInternalPath($item),
            'time'    => date('Y-m-d h:i', $mtime),
            'type'    => '',//$file_type,
            'icon'    => $icon,
            'thumb'   => $thumb_url,
            'is_file' => $is_file,
    ...
    public function createFolderByPath($path)
    {
        $disk = $this->getDiskToStore();
        if (! $disk->exists($path)) {
            $disk->makeDirectory($path, config('lfm.create_folder_mode', 0755), true, true);
        }
    }
    ...
    {
        $disk = $this->getDiskToStore();
        return count($disk->allFiles($directory_path)) == 0;
    }
     ...
     */
    public function makeImageProcess($content, $new_filename) {
        $new_file_path = $this->getCurrentPath();
        $new_file_path = $this->updatePath($new_file_path); // replace files to photos into path
        $disk = $this->getDiskToStore();

        $disk->put($new_file_path . $this->ds . $new_filename, $content);
        // Generate a thumbnail
        if (config('lfm.should_create_thumbnails', true)) {
            $this->makeCkeditorThumbImage($new_filename, $content);
        }
        return $new_filename;
    }
    /*
     * make thumb images for ckeditor
     */
    private function makeCkeditorThumbImage($new_filename, $file = null) {
        // create thumb folder
        ...
        $temp_path = $this->updatePath($temp_path); // replace files to photos into path
        // create thumb image
        Image::make($file)
        ...
        $thumbPAth = $this->updatePath($thumbPAth); // replace files to photos into path
        $disk->putFileAs($thumbPAth, new FileObj($temp_path), $new_filename, 'public');
        //delete local temp path
        File::delete($temp_path);
    }
    //update path from library
    private function updatePath($filepath, $replaceWith = 'files') {
        return str_replace($replaceWith, config('lfm.images_folder_name'), $filepath);
    }
    //resize image with max height and width
    private function resizeImageResponsive($dimension, $maxHeight, $maxWidth) {
        $originalHeight = isset($dimension[1]) ? $dimension[1] : config('constants.ckeditor_max_height');
        $originalwidth = isset($dimension[0]) ? $dimension[0] : config('constants.ckeditor_max_width');
        $ww = $maxWidth;
        $hh = $maxHeight;
        $match2 = (int) trim($originalwidth);
        $width = ($match2 > $ww) ? $ww : $originalwidth;
        $match4 = (int) trim($originalHeight);
        $height = ($match4 > $hh) ? $hh : $originalHeight;
        if ($width < $ww && $height == $hh) {
            $width = "auto";
        } else if ($width == $ww && $height < $hh) {
            $height = "auto";
        }
        $heightPx = 'px';
        if ($height == "auto") {//if auto then no need to add px after that
            $heightPx = '';
        }
        $widthPx = 'px';
        if ($width == "auto") {//if auto then no need to add px after that
            $widthPx = '';
        }
        return 'height="' . $height . $heightPx . '" width="' . $width . $widthPx . '"';
    }
    ...
    */
    public function getFileDimension($file, $new_filename) {
        //create thumb image in local and then move that file to file storage and then delete local file.
        $temp_path = $this->translateToOsPath($this->getLocalUploadPath() . $this->ds. "tempdim".time() . $new_filename);
        $temp_path = $this->updatePath($temp_path); // replace files to photos into path
        // create thumb image
        Image::make($file)
            ->save($temp_path);
        //get file dimmenions using absolute path
        $dimension = getimagesize($temp_path);
        //delete local temp path
        File::delete($temp_path);
        return $dimension;
    }
